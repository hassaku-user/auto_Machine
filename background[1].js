    ...
    try {
        await chrome.scripting.executeScript({
            ...
            func: (subjectInsStr, headRange, insertText, countWord) => {
                ...
                if (countIndex !== -1) {
                    const deleteStart = Math.max(countIndex - 2, 0);
                    const deleteEnd = deleteStart + 66; // 39行分
...
async function runCheckMailForID(rawID) {
    ...
    // URL 作成
    const targetURL = urlCreate(rawID);
    if (!targetURL) return;
    ...
    await chrome.scripting.executeScript({
        ...
        func: (procItem) => {
            ...
            // ① Aタブの table を取得
            const table = document.querySelector(".appli-basic");
            if (!table) return;

            // tdをすべて取得して1つの文字列にまとめる
            const tds = Array.from(table.querySelectorAll("td")).map(td => td.innerText);
            const tdString = tds.join("\n");

            console.log(tdString);

            // ② /区切りで配列にして rsvDate に格納
            let rsvDate = tdString.split("\n")
                .filter(text => /^・\d{4}/.test(text)) // 「・XXXX～」で始まるものだけ
                .map(s => s.trim()); // 文頭・文末の空白を削除

            console.log(rsvDate);

            // ③ rsvDate 内の文字列を / で分割
            let rsvDateParts = rsvDate.map(s => s.split("/").map(part => part.trim()));

            // ④ procItem によって対象日付を選択
            const idxMap = { "区間1": 0, "区間2": 1, "区間3": 2, "区間4": 3 };
            const idx = idxMap[procItem] ?? 0;

            // rsvDateParts[idx] が配列なので [0], [1] で date1 / date2 を取得
            const date1 = rsvDateParts[idx]?.[1] || "";
            const date2 = rsvDateParts[idx]?.[2] || "";

            // ⑤ memo_text の先頭に追加
            const existingValue = memoText.value;
            const newValue = `${procItem}元\n${date1}\n(${date2})\n\n${existingValue}`;
            memoText.value = newValue;

            // confirm_button は押さない
...
chrome.tabs.onUpdated.addListener(async function (tabId, changeInfo, tab) {
    if (changeInfo.status === "complete" && tab.url) {
        ...
        if (tab.url === expectedURL || tab.url.startsWith(expectedURL)) {
            // 一致したら処理開始
            const headDelete = { from: 0, to: 6 }; // 文書先頭から3〜5行目を削除
